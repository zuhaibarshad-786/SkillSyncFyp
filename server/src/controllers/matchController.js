...
exports.getMatchSuggestions = asyncHandler(async (req, res) => {
    ...
    if (suggestions.length === 0) {
        return res.json({ 
            message: 'No complementary matches found yet. Update your listing!',
            suggestions: [] 
        });
    }
...
exports.acceptMatch = asyncHandler(async (req, res) => {
    ...
    // NEW LOGIC: Determine what skills are being swapped for the record
    const listA = await Listing.findOne({ user: userAId });
    const listB = await Listing.findOne({ user: userBId });

    // Find the overlap (A learns from B's teaching list)
    const skillForA = listA.skillsToLearn.find(s => 
        listB.skillsToTeach.some(t => t.name === s.name)
    );

    // Find the overlap (B learns from A's teaching list)
    const skillForB = listB.skillsToLearn.find(s => 
        listA.skillsToTeach.some(t => t.name === s.name)
    );

    const newMatch = await Match.create({
        userA: userAId,
        userB: userBId,
        skillLearnedByA: skillForA ? skillForA.name : 'General Exchange',
        skillLearnedByB: skillForB ? skillForB.name : 'General Exchange',
        status: 'active'
    });
